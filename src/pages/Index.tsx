...
import MoodFlowerQuiz from "@/components/MoodFlowerQuiz";
...
const Index = () => {
  ...
  const moodSection = useFadeInOnScroll();
  ...
  const features = [
    { icon: <Flower2 size={28} strokeWidth={1.5} />, title: "Ruh Haline Göre Çiçek", description: "Nasıl hissediyorsan, ona uygun çiçeği buluyoruz." },
    { icon: <Truck size={28} strokeWidth={1.5} />, title: "Aynı Gün Teslimat", description: "Siparişiniz aynı gün kapınıza özenle teslim edilir." },
    { icon: <Heart size={28} strokeWidth={1.5} />, title: "Taze Garanti", description: "Tüm çiçeklerimiz taze ve kaliteli olarak garanti altındadır." },
    { icon: <Sparkles size={28} strokeWidth={1.5} />, title: "Kişiye Özel Notlar", description: "Her siparişe özel mesaj kartı ile duygularınızı ifade edin." },
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/60 backdrop-blur-xl border-b border-border/50 shadow-sm">
        <div className="container mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8">
            <a href="#mood" className="text-sm text-foreground/70 hover:text-foreground transition-colors">Çiçeğini Bul</a>
      ...
      <section className="relative min-h-[85vh] sm:min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 pt-20 pb-24 sm:pb-10">
          <div className="max-w-2xl">
            ...
            >
              Find the flower that<br />
              <span className="italic text-primary">matches</span> your mood.
            </h1>
            <p
            ...
            >
              Ruh haline göre sana özel çiçek önerileri. Flora, duygularını çiçeklere dönüştürüyor.
            </p>

            <div className="opacity-0 animate-fade-in flex flex-col sm:flex-row gap-4" style={{ animationDelay: "0.7s" }}>
              <Button
                size="lg"
                className="btn-glow bg-primary hover:bg-primary/90 text-primary-foreground h-12 px-8 rounded-full transition-all duration-300 hover:shadow-[0_10px_30px_-10px_hsl(350_50%_70%)] flex items-center gap-2"
              ...
              <Button
                size="lg"
                variant="outline"
                className="h-12 px-8 rounded-full border-border hover:bg-secondary/50"
                onClick={() => document.getElementById('waitlist')?.scrollIntoView({ behavior: 'smooth' })}
              >
      ...
      {/* Mood → Flower Quiz Section */}
      <section id="mood" className="py-16 sm:py-24 bg-secondary/20">
        <div
          ref={moodSection.ref}
          className={`container mx-auto px-4 sm:px-6 transition-all duration-1000 ${
            moodSection.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}
        >
          <div className="text-center mb-10 sm:mb-14">
            <span className="text-sm tracking-[0.2em] uppercase text-primary mb-3 block">Mood → Flower</span>
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground mb-4">
              Ruh Haline Göre Çiçek
            </h2>
            <p className="text-foreground/60 max-w-md mx-auto text-sm sm:text-base">
              Bugün nasıl hissediyorsan, doğa sana cevap versin
            </p>
          </div>
      ...
      {/* Features */}
      <section id="ozellikler" className="py-16 sm:py-24 bg-background">
        <div
        ...
        >
          <div className="text-center mb-12 sm:mb-16">
            ...
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground mb-4">Farkımız Ne?</h2>
            <p className="text-foreground/60 max-w-md mx-auto text-sm sm:text-base">Her detayı düşünerek, size en güzel deneyimi sunuyoruz</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {features.map((feature, index) => (
              <div key={index} className="group bg-card rounded-2xl p-6 sm:p-8 text-center card-shadow transition-all duration-500 hover:-translate-y-2" style={{ transitionDelay: `${index * 100}ms` }}>
      ...
      {/* Collections */}
      <section id="koleksiyonlar" className="py-16 sm:py-24 bg-secondary/20">
        <div
        ...
        >
          <div className="text-center mb-12 sm:mb-16">
            ...
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground mb-4">Çok Yakında Sizlerle</h2>
            <p className="text-foreground/60 max-w-md mx-auto text-sm sm:text-base">Lansmanla birlikte açılacak olan koleksiyonlarımızdan bir önizleme</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {collections.map((item, index) => (
              <div key={index} className="group relative rounded-2xl overflow-hidden card-shadow aspect-[3/4] cursor-pointer">
                ...
                <div className="absolute top-3 right-3 bg-background/80 backdrop-blur-sm text-foreground text-[10px] sm:text-xs font-medium px-3 py-1 rounded-full">Yakında</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* About */}
      <section id="hakkimizda" className="py-16 sm:py-24 bg-background">
        <div
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="lg:pl-8">
              ...
              <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground mb-6 leading-tight">Duygularınızı Çiçeklerle İfade Ediyoruz</h2>
              <div className="space-y-4 text-foreground/70 leading-relaxed">
                <p>Her duygunun bir çiçeği var. Flora olarak, ruh halinize uygun çiçekleri seçerek yaşamınıza renk katmayı amaçlıyoruz.</p>
                <p>Mutluluk, huzur, aşk ya da motivasyon — hangi duyguyu yaşıyorsanız, doğanın o duyguya en yakın armağanını sizin için hazırlıyoruz.</p>
              </div>
              <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-border">
                <div className="text-center">
                  <p className="font-display text-2xl sm:text-3xl text-primary">4</p>
                  <p className="text-sm text-foreground/60">Ruh Hali</p>
                </div>
                <div className="text-center">
                  <p className="font-display text-2xl sm:text-3xl text-primary">50+</p>
                  <p className="text-sm text-foreground/60">Çiçek Çeşidi</p>
                </div>
                <div className="text-center">
                  <p className="font-display text-2xl sm:text-3xl text-primary">∞</p>
                  <p className="text-sm text-foreground/60">Duygu</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 sm:py-24 bg-secondary/20">
        ...
        >
          <div className="max-w-3xl mx-auto text-center">
            <Quote size={40} className="text-primary/40 mx-auto sm:w-12 sm:h-12 mb-6 sm:mb-8" strokeWidth={1} />
            <blockquote className="font-display text-xl sm:text-2xl md:text-3xl lg:text-4xl text-foreground leading-relaxed mb-6 sm:mb-8 px-4">
              "Huzurlu hissediyorum dedim, lavanta buketi geldi. Hem kokusu hem görüntüsü muhteşemdi. Tam bana göre!"
            </blockquote>
            <div className="flex flex-col items-center gap-3">
              <p className="text-base sm:text-lg font-medium text-foreground">Elif K.</p>
      ...
      <section id="waitlist" className="py-20 sm:py-28 bg-primary/10 relative overflow-hidden">
        ...
        >
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground mb-4">Lansman Günü Haberdar Olun</h2>
            <p className="text-foreground/60 mb-8 sm:mb-10 text-sm sm:text-base max-w-md mx-auto">
              Bekleme listesine katılın, ruh halinize göre çiçek önerilerimizi ilk siz keşfedin ve <span className="text-primary font-medium">%20 özel indirim</span> kazanın!
            </p>
            {isSubmitted ? (
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-8">
            <div className="flex flex-col items-center md:items-start gap-4">
              ...
              <p className="text-background/60 text-sm italic">Find the flower that matches your mood.</p>
              <a
            ...
            <div className="flex items-center gap-3 text-background/80">
              <Clock size={18} />
              <p className="text-sm">Lansman: Yakında</p>
            </div>
            <div className="text-sm text-background/60 text-center">© 2026 Flora. Tüm hakları saklıdır.</div>
          </div>
        </div>
      </footer>
    </div>
